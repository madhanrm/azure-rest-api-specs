...
into every dispatch prompt (and every session-handoff paste). The
...
- The `PR:` header field exactly matches the current PR.
- The `Head SHA:` header field exactly matches the session SHA pinned in
  Reviewer Step 1.
- The `Iteration:` header field is `1`-`3`.
- The pasted output begins with a valid `<!-- critic-verdict: ... -->`
  marker (literal first line) whose `finding=` value matches the
  written verdict in the body.
- The pasted output includes the per-finding annotations the Reviewer
  needs to fold corrections in.

If any check fails, the Reviewer rejects the pasted output and requests
a corrected verbatim paste. Free-form acknowledgments ("looks fine") are
...
| `iteration`   | `1`-`3` (Critic iteration this response reflects); `0` for Reviewer-detected drift between iterations | Echo of the `Iteration:` input for the most recent Critic call. See note below on `iteration=0`.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
`<!-- critic-verdict: ... -->` marker, **even on early-abort paths**
